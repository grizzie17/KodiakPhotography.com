...
FUNCTION needsScaling( h, w )
	...
	IF nW < w  OR  nH < h THEN EXIT FUNCTION
	
	IF w <= nW THEN
		IF nH = h THEN needsScaling = FALSE
	ELSEIF h <= nH THEN
		IF nW = w THEN needsScaling = FALSE
	END IF
...
IF "" <> cType THEN
	...
	IF NOT oCanvas IS Nothing THEN
		...
		IF IsNum(q_h)  AND  IsNum(q_w) THEN
			...
			IF 0 < LEN(sCopy)  AND  " " <> sCopy THEN
				DIM	x, y
				oCanvas.Canvas.Font.Bold = 1
				oCanvas.Canvas.Font.Size = 16
				x = 5
				y = h - (5+16)
				oCanvas.Canvas.Font.Color = &H666666
