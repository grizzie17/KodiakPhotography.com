...
FUNCTION genSortnameDeltaFromNow( d )
	...
	IF ISDATE( d ) THEN
		DIM dNowTZ
		dNowTZ = DATEADD("h", g_nTimeZoneOffset, NOW )
		DIM n
...
SUB buildFileList( sPath, bDynSortname )
	buildFileListX nFileCount, aFileList, sPath, bDynSortname
END SUB

SUB buildFileListX( nFileCount, aFileList(), sPath, bDynSortname )
	...
	FOR EACH oFile IN oFolder.Files
		...
		IF useFileName( sName ) THEN
			...
			IF "" <> sDateRange THEN
				...
				ELSE
					sDateBegin = DATEVALUE(DATEADD("h", g_nTimeZoneOffset, oFile.DateCreated))
					sDateEnd = sDateRange
				END IF
