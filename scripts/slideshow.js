...
CSlideScreen.superclass = CSlideBox.prototype;
...
CSlideScreenTransition.superclass = CSlideScreen.prototype;


CSlideScreenTransition.prototype.isValidFilter
= function( sFiltername )
{
	var	bValid = false;
	var oBox = this.getBox();
	try
	{
		if ( typeof( oBox.filters ) != "undefined" )
		{
			if ( 0 < oBox.filters.length )
			{
				if ( typeof( oBox.filters.item(sFiltername) ) == "object" )
				{
					if ( null != oBox.filters.item(sFiltername) )
					{
						var o = oBox.filters.item(sFiltername);
						var sApply = typeof( o.apply );
						var sFunc = typeof( o.play );
						//window.alert( "apply type = " + sApply + "; play type = " + sFunc );
						//if ( typeof(oBox.filters.item(sFiltername).Play) != "undefined" )
						if ( sFunc != "undefined" )
							bValid = true;
					}
				}
...
= function()
{
	...
	{
		this.m_bFilters = false;
		if ( this.isValidFilter( this.m_sFiltername ) )
		{
			this.m_bFilters = true;
		}
		else if ( this.isValidFilter( 0 ) )
		{
			this.m_bFilters = true;
			this.m_sFiltername = 0;
		}
		return true;
	}
	else
...
= function()
{
	if ( this.m_bFilters )
	{
		if ( typeof( this.m_oDiv.filters.item(this.m_sFiltername).duration ) == "undefined" )
			return 0;
		else
			return this.m_oDiv.filters.item(this.m_sFiltername).duration;		// return as seconds
	}
	else
	{
		return 0;
...
= function()
{
	if ( this.m_bFilters )
	{
		if ( "undefined" == typeof(this.m_oDiv.filters.item(this.m_sFiltername).status) )
			return 0;
		else
			return this.m_oDiv.filters.item(this.m_sFiltername).status;
	}
	else
	{
		return 0;
...
= function( sSlideValue )
{
	...
	CSlideScreen.prototype.displaySlidePreprocess.call( this, sSlideValue );
	...
	if ( 0 < this.getTransitionDelay() )
	{
		...
		try
		{
			this.m_oDiv.filters.item(this.m_sFiltername).apply();
		}
		catch ( e )
		{
			try
			{
				this.m_oDiv.filters.item(0).apply();
				this.m_sFiltername = 0;
			}
			catch ( e )
			{
				window.status = "preprocess failed";
			}
		}
	}
};
...
= function( sSlideValue )
{
	if ( 0 < this.getTransitionDelay() )
	{
		try
		{
			this.m_oDiv.filters.item(this.m_sFiltername).play();
		}
		catch ( e )
		{
			try
			{
				this.m_oDiv.filters.item(this.m_sFiltername).play(1);
			}
			catch ( e )
			{
				window.status = "postprocess failed";
			}
		}
	}
	CSlideScreen.prototype.displaySlidePostprocess.call( this, sSlideValue );
...
CSlideScreenTransitionRandom.superclass = CSlideScreenTransition.prototype;
...
function CSlideProjector()
{
	...
	this.cancelTimeout = function( nTimerID )
	{
		clearTimeout( nTimerID );
	};
	
	
	this.cancelRunTimer = function()
	{
		if ( -1 < this.m_nRunTimer )
			this.cancelTimeout( this.m_nRunTimer );
		this.m_nRunTimer = -1;
	};
...
CSlideProjectorShowSingle.superclass = CSlideProjector.prototype;



CSlideProjectorShowSingle.prototype.stopRunning
= function( e )
{
...
= function()
{
	...
	window.onbeforeunload = (function(e){e = e || window.event;tempThis["stopRunning"](e,this)});
	window.onunload = (function(e){e = e || window.event;tempThis["stopRunning"](e,this)});
...
= function()
{
	...
	if ( 0 == this.m_nBlendDelay )
		this.m_nRunTimer = this.setTimeout( "waitOnTransition", 0.1 );
...
= function()
{
	...
	this.m_nRunTimer = this.setTimeout( "pauseShow", this.m_nPicturePause );
};
...
= function()
{
	...
	if ( d < this.m_dPauseTarget )
	{
		...
		this.m_nRunTimer = this.setTimeout( "pauseShow", (tt-t)/1000 );
	}
	else
	{
		this.setTimeout( "makeReadyForShow", 0.01 );
...
function CSlideCarousel()
{
	...
	this.m_nCookerTimerID = -1;	// setTimeout
	...
	this.cancelCookerTimer = function()
	{
		if ( -1 < this.m_nCookerTimerID )
			this.cancelTimeout( this.m_nCookerTimerID );
		this.m_nCookerTimerID = -1;
	};
...
= function()
{
	this.cancelCookerTimer();
	this.m_nCooker ++;
	if ( this.m_nCooker < this.m_aSlideCards.length )
	{
		this.m_aSlideCards[this.m_nCooker].precook();
		this.m_nCookerTimerID = this.setTimeout( "precookCards", this.m_nCookerTimer );
	}
};
...
CSlidePicture.superclass = CSlideCard.prototype;
...
CSlideHTML.superclass = CSlideCard.prototype;
